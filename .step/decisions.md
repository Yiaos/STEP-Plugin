...
- **日期**: 2026-02-15
- **状态**: accepted
- **决策**: state.yaml 头部增加 ⚡ 行为规则注释，PreToolUse hook 通过 `cat | head -25` 同时注入规则和数据
...
- **日期**: 2026-02-15
- **状态**: accepted
- **决策**: 不单独建 research/ 目录（decisions.md 已承载决策推理）；不做 git diff 推断恢复（state.yaml 的 progress_log + next_action 已够用）
